...
insert into _commune (code_insee_de_la_commune, nom_de_la_commune, code_du_departement)
  ...
  from _temp_fr_en_etablissements_ep
  where (code_commune, nom_commune) not in (select code_insee_de_la_commune, nom_de_la_commune from _commune);
  
-- Il manque des communes dans _communes
----------------
--- _academie
...
ALTER TABLE _quartier_prioritaire ADD constraint quartier_prioritaire_pk primary key(code_quartier_prioritaire,nom_quartier_prioritaire);

ALTER TABLE _est_a_proximite_de ADD nom_quartier_prioritaire    varchar(85);

ALTER TABLE _est_a_proximite_de ADD constraint est_a_proximite_de_fk_qp
        foreign key(code_quartier_prioritaire,nom_quartier_prioritaire)
...
select distinct qp_a_proximite, nom_du_qp
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite, nom_du_qp) is not null and (qp_a_proximite, nom_du_qp) not in (select code_quartier_prioritaire, nom_quartier_prioritaire from _quartier_prioritaire);
...
select distinct code_nature, libelle_nature
  from _temp_fr_en_etablissements_ep
  where (code_nature, libelle_nature) is not null;
...
select distinct uai, nom_etablissement, statut_public_prive, code_postal, latitude, longitude, code_commune, nom_commune, code_academie, code_nature
  from _temp_fr_en_etablissements_ep
  where (uai, nom_etablissement, statut_public_prive, code_postal, latitude, longitude, code_commune, nom_commune, code_academie, code_nature) not in (select uai, nom_etablissement, secteur, code_postal, latitude, longitude, code_commune, nom_commune, code_academie, code_nature from _etablissement)
...
select distinct qp_a_proximite, uai, nom_du_qp
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite, uai, nom_du_qp) is not null and (qp_a_proximite, uai, nom_du_qp) not in (select code_quartier_prioritaire, uai, nom_quartier_prioritaire from _est_a_proximite_de) and qp_a_proximite != 'Hors QP' and uai in (select uai from _etablissement);
...
select distinct rentree_scolaire
  from _temp_fr_en_college_effectif_niveau_sexe_lv
  where rentree_scolaire is not null;
...
-------------------------
-- Caractéristiques_tout_etablissement
...
select distinct e.uai, c.rentree_scolaire, e.nombre_d_eleves
  from _temp_fr_en_etablissements_ep as e
  join _temp_fr_en_college_effectif_niveau_sexe_lv as c on e.uai = c.numero_college
  where (e.uai, c.rentree_scolaire, e.nombre_d_eleves) is not null
  and (e.uai, c.rentree_scolaire, e.nombre_d_eleves) not in (select uai, annee_scolaire, effectifs from _caracteristiques_tout_etablissement);
